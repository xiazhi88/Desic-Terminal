...
Only the desktop-owned strategy configuration can mark a parameter as a future tuning candidate. It may configure `min`, `max`, and `step` only for an existing top-level finite numeric `ctx.params` value. Python source cannot declare itself optimizer-eligible, cannot change a parameter value, and cannot add a tuning range. Parameter search is intentionally not run by this version of the runtime; the contract exists so future optimization results can remain reproducible.
...
`Bar` has `openTimeMs`, `closeTimeMs`, `open`, `high`, `low`, `close`, `volume`, and `confirmed`. All 1-minute bars are confirmed. For higher intervals, only the final item can be `confirmed=False`; its OHLCV contains only minutes known at the callback. Use the preceding item where a confirmed higher-timeframe signal is required.
...
The Strategy Research AI button opens a right-side multi-turn assistant for the currently selected Python strategy. It uses a Desic runtime-scoped authoring Skill rather than a one-shot code-generation prompt. Each turn first calls `strategy.readCurrentSource`, which returns the live unsaved editor buffer and a revision number; it cannot read another strategy, a filesystem path, credentials, market data, or an account.

For a requested source change, the assistant must call `strategy.applySource` with a complete replacement file and the revision it just read. The desktop rejects stale revisions, writes the accepted source into the visible editor with the normal review animation, and reports the new revision to the assistant. This is not a save, version creation, Python execution, backtest, optimization, or order action. The user reviews the change and saves a version explicitly. Switching strategy, closing the panel, stopping generation, or manually editing while a write is in progress cancels the scoped session or the pending write.

After every successful source write, the assistant must call `strategy.testCurrentSource`. This reads the latest unsaved buffer, statically locates every `ctx` action call with its source line, and runs one `on_start` (when present) and one natural `on_bar` event through the same local JSONL Python protocol used by Profiles. The fixture is deterministic, contains 240 closed bars for every supported interval, uses saved parameters and an empty virtual portfolio, and never changes conditions to force a trade. A failure returns the Python or output-contract error for the assistant to repair; a statically checked call site may remain runtime-unreached. A pass only proves source policy and reached output paths completed and does not replace a historical backtest or prove every branch, fill, profitability, or live safety.

The authoring Skill receives saved parameters and platform-owned tuning ranges only as immutable reference data. It cannot alter parameter JSON or optimizer eligibility. It must explain and comment source in the current interface language and use the same current-time, market, portfolio, and action contract described above.
