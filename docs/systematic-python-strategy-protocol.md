...
The Strategy Research AI button opens a right-side multi-turn assistant for the currently selected Python strategy. It uses a Desic runtime-scoped authoring Skill rather than a one-shot code-generation prompt. Each turn first calls `strategy.readCurrentSource`, which returns the live unsaved editor buffer and a revision number; it cannot read another strategy, a filesystem path, credentials, market data, or an account. Before creating or changing source, it must call the read-only `strategy.readDevelopmentDocs` tool. That tool returns this complete, versioned document together with `documentationVersion`, `protocolVersion`, and a SHA-256 content digest. The embedded document is the authoritative contract for the current editor session; it is not an arbitrary file reader and cannot access market, account, credential, or other strategy data.
