...
- Cline 的 local runtime 只在 sidecar 进程内保留 active session；普通交互会话和策略编辑会话必须显式用 `interactive: true`，同一进程内通过 `send` 续接。应用退出时先向 sidecar 发送 `shutdown` 并等待 Cline dispose，避免把已完成会话留给 stale-session reconciler。重启 sidecar 后，先从 Cline 的 `readMessages` 读取原始 artifact，以相同 session ID 调用无 prompt 的 `start({ initialMessages })` 重建 runtime，最后单独 `send` 新消息；不能把 `initialMessages` 与新 prompt 放在同一次 `start` 中。Cline 的 `idle`/`completed`/`failed`/`running`/`cancelled` 等状态都属于进程元数据，不能单独决定历史是否可恢复；scope/policy 匹配后，Desic 不因中断的 tool call、缺少最终 assistant 文本、空 artifact 或旧进程失败而拒绝恢复，具体 artifact 解释和继续行为交给 Cline 自己处理。恢复前必须核对策略/账户/权限/工具策略 scope，后台、Review 和一次性生成任务仍保持非交互式终止语义。
- AI provider watchdog 必须限制连续无事件的 idle 时长，不能把一次 turn 的总运行时间当作绝对 deadline；任何 Cline/provider 状态、文本、reasoning、工具或 usage 活动都应刷新 idle 起点。带响应体的 HTTP 错误继续立即失败，不能等 watchdog；已有模型文本或工具进度后也不能自动重放整轮，以免重复副作用。本机 Codex/Claude CLI 由受监督子进程的退出、JSON-RPC、Provider 和 CLI 错误决定终态，不使用外层网络 idle watchdog：工具结果回传后的下一次模型请求可能长时间没有可映射事件，不能在 CLI 返回具体结果前把正常静默推理误杀为 `Request timed out.`；测试可显式传入短 timeout 覆盖此规则。Provider 网络瞬态错误沿用 DSH retry policy 的默认值：最多 5 次重试，500ms 起步、指数退避至 10s、10% 抖动；收到已有模型/工具进度后不自动重放整轮。idle 看门狗超时不属于「瞬态网络错误」：它由我们自己的 `code = "provider_idle_timeout"` 标记，自 2026-09-18 起**不参与自动重试**（大上下文下重试会重发整份上下文，时长与费用都是浪费，且触发条件通常可复现），直接以诊断文案收尾并提示用户换更快的模型或稍后手动重试；provider 自己返回的 `Request timed out.`（不带该 code）必须原样透传，不得改写。对客文案是中文诊断串（`模型连续 N 秒没有输出，已中止该次请求…`），且**必须与尝试次数无关** —— `retryableNetworkError` 与最终 `errorMessage` 之间存在逐字相等的断言。连续无事件的 idle 上限默认 240s（显式 `requestTimeoutMs` 优先、上限 10 分钟；本机 Codex/Claude CLI 不受此限）；旧的 60s 默认值曾在真实 Profile 运行中把「续字间隔超过 60s 的大上下文请求」误杀成 `Request timed out.`，不要再退回。
- 必需 Skill（如 `trading-philosophy`）的内容由用户编辑并保存在本地配置中，属于“用户可改、AI 不可直改”。模型侧不存在任何 Skill 内容写工具；`skills`/`skill.readResource`/`skill.run` 只负责加载、读取资源和运行入口。AI 参与 Skill 优化的唯一合法路径是复盘运行中的 `review.readSkillVersion` + `optimizationSuggestion.create`：只提交带完整 `proposedSkill` 的候选修改供人工预览，不直接变更生效 Skill。不要为“方便”引入 `skill.write` 之类的直改工具；即使 open-agent 模式允许 Cline 原生文件工具，Skill 运行时内容也来自配置存储而非工作区文件，文件写入不等于修改 Skill。
...
- 参数调优必须只消费桌面保存的顶层数值范围。开始调优前固定策略版本、70/30 训练验证区间和 30/100/300 候选预算；超过预算的组合使用由策略版本与范围共同派生的确定性抽样，并将候选模式、预算、种子和基线验证指标持久化，保证结果可复现且不会悄悄改变实盘 Profile。
...
- Codex CLI 属于 provider-executed tool runtime：不能把其 tool-call chunk 再交给 Cline AgentRuntime 执行，否则会产生二次执行或 `Tool execution is disabled`。应用工具必须经带随机认证的 loopback MCP 映射到已有 `AgentTool.execute`，保留 JSON Schema、Sidecar policy 和 Rust 最终授权；交互请求必须使用 `codex app-server --listen stdio://`，等待同一 turn 的 `turn/completed`，并实时转发 `item/agentMessage/delta` 与 reasoning delta；禁止用 `codex exec --json` 冒充流式接口，因为它会把 `assistant_message` 缓冲到进程退出后才一次性输出。Codex app-server 协议以本机 `generate-json-schema --experimental` 为准；第三方 AI SDK provider 如果把官方带默认值的可选字段误设为必填，会先显示 delta、再丢弃 `item/completed`/`turn/completed` 并永久 running。launcher 边界必须逐行解析 JSON-RPC，补齐 `phase`、turn `error`、reasoning 数组、MCP 结果与 token usage 等兼容默认值；终态 turn 中未知的新 item 只能局部忽略，不能让整条终态通知校验失败。app-server 必须保留调用者的 `CODEX_HOME/HOME`，不得读取、复制或解析 `auth.json`：该文件可能承载 API Key，也可能承载会轮换的 OAuth access/refresh token，凭据还可能位于 OS keyring。运行使用 stateless ephemeral thread，注入当轮 Desic MCP，并通过 `thread/start.config` 显式覆盖 Desic 管理的思考深度、能力开关、Provider 路由和本轮 MCP 授权；`model_reasoning_effort` 必须在 Codex 反序列化用户配置前覆盖为 CLI 支持的枚举，不能让个人 `config.toml` 中的同名值否决应用内选择。Codex 原生 shell/Web Search/原生 Agent 继续关闭；仍需使用 exec 的一次性诊断必须强制带 `--ignore-user-config --ignore-rules --ephemeral`。非交互运行使用 `approval_policy=never` 时会自动拒绝默认需要确认的 MCP 调用，因此只能对本轮已通过 Desic 权限策略、且由随机 Token 保护的临时 MCP 显式设置 `default_tools_approval_mode=approve`；不得关闭只读 sandbox 或扩大暴露工具集合。由于 `--ignore-user-config` 同时会忽略用户自定义模型路由，Tauri 只能从 `CODEX_HOME/config.toml` 白名单读取当前 `model_provider` 的非敏感路由字段，并以 `-c` 覆盖重建 Provider；不得透传静态请求头、查询参数、Token 或其它用户配置。不能只用 `mcp_servers.<plugin>.enabled=false` 禁用插件 MCP，因为插件 transport 尚未进入基础配置时会形成无 transport 的无效配置；也不能借本机 CLI 绕过应用工具边界。launcher 还必须监督 `turn/started` 到 `turn/completed`/不可重试 `error` 的闭合关系；子进程在活跃 turn 未闭合时退出必须合成具体 JSON-RPC error，不能依赖任意请求超时或让 Cline 永久等待。Provider 错误识别只能依据结构化 JSON-RPC error 或带明确 HTTP 错误上下文的 stderr，禁止在完整 stdout 中按 `500` 等普通数值做全局匹配。
...
- `desic-core-operations`、`trading-philosophy`、`okx-market-intelligence`、`market-radar-research`、`desic-trade-operations`、`desic-agent-orchestration` 是全局必需 Skills：界面必须显示为已启用且不可关闭，Rust 保存和读取配置时必须补齐五个显式 Skill；`desic-core-operations` 继续作为不可编辑的隐式固定规范，不写入 `enabledSkills`。`desic-trade-operations` 合并交易机会、市场证据、永续风险、仓位生命周期与保护核对；`okx-market-intelligence` 合并新闻、事件、情绪、宏观、Smart Money 与衍生品情报；`market-radar-research` 约束全市场排行、归因、广度、保存筛选和点时验证的只读解释；`desic-agent-orchestration` 约束多 Agent 调度纪律（主 Agent 从本次勾选名单里点名、任务下达、按需追问、报告按不可信证据合并与冲突处置；不再有后端预先成波或自动复核波，复核专家只是"可勾选、可点名"的普通 Agent）。旧 Skill ID 只作为读取迁移别名，不得重新出现在新的默认配置或工具路由中。`trading-philosophy` 必需但可定制，升级默认内容时只能迁移完全未修改的旧内置版本，必须保留用户自定义内容；其默认理念约束证据、风险和可修正性，不得把具体指标、周期、盈亏比或风险比例固化为普适规则。必需 Skill 集合的跨语言副本（前端 `App.tsx`、前端 `AiAutomationPanel.tsx`、Rust `storage-config`/`ai_automation`、`smoke-terminal-preview` 冒烟清单）由 `npm run test:ai-skills-sync` 锁定为同一 6 项规范集；修改任一副本时必须同步并运行该测试。
- 默认系统 Prompt 与六个内置 Skill 的基线统一维护在 `shared/default-ai-config.json`，Rust、前端和 Sidecar 不得各自复制一份。默认基线升级只能按旧内容精确匹配或指纹迁移；任何用户编辑过的 Prompt 或 Skill 都必须原样保留。
...
- AI 流式 reasoning/text 落库时应合并相邻 delta，并在工具、代理或审批事件处保留边界；Codex app-server 的 reasoning summary 还必须保留 `itemId`、`summaryIndex` 与 `isSummary`，只合并同一 item、同一 summary index 的 delta，并作为独立过程摘要呈现，不能和详细 reasoning 或其它 summary 拼成一个“思考过程”块。历史加载端只按持久化事件类型恢复，不能通过正文内容猜测或修剪过程文本，也不能假设旧 `tool_json` 已经压缩。
...
- 交互会话运行中追加问题必须使用 Cline 原生 `send({ delivery: "queue" | "steer" })`，不能先 abort 当前回答或在前端伪造队列。`run-started` 同时可能来自 Provider 网络重试；只有与此前真实提交的 pending prompt 匹配时才能切分并持久化下一轮，首次 run 和未匹配重试必须忽略。pending prompt 的提交、更新或删除失败是非终止命令错误，不得把活跃回答误判为失败；其身份匹配必须遵循 Cline 的原始文本精确相等与 native prompt ID，不能用 `trim()` 合并空白不同的消息，更新/删除命令也必须等 Core 实际确认后才能返回成功；前端列表刷新必须按 session 做请求排序，并由 native pending 事件和本地乐观变更使旧响应失效；Stop 必须同时清理 Cline pending prompt 并将本地 `streaming/queued/steering` 消息标为取消。
- 正常 AI `Done` 必须在最终 assistant 文本、过程事件、权威 usage 和 session 状态成功提交后再发给 UI，避免“界面已完成但消息尚未落盘”的分支、重载和 usage 竞态。消息分支只复制选中消息之前的持久化会话历史并创建新的 Cline continuation；它不是 workspace checkpoint，不得删除源历史或回滚已发生的文件、工具、交易和外部副作用。当前上下文占用是按 session 保存的状态，只能使用 Cline `readMessages` + `getCurrentContextSize` 的实测值，不能挂到“最后一条 assistant 消息”来猜测归属；窗口上限优先使用与 provider/model 精确匹配的 Cline 模型目录，未列出的模型再使用经过正整数校验的显式 `contextWindow`，仍未知时使用明确标识的 `256_000` 默认回退。system/tool/conversation 分类仅是本地 heuristic，不能等同 Provider 计费或作为发送门槛；TTFT 只在实际观察到首个 text/reasoning delta 时显示。交互 Cline session 的 native `compaction` 可在下一次 provider 请求前执行；配置必须保持自动、基本压缩、90% 阈值、保留近期窗口，并且只缩减模型可见请求历史，绝不能改写 Desic 持久化消息、工具审计、权限、会话 ID 或未完成回合。没有 SDK 可验证的手动压缩入口时，不得注册或显示一个会谎报“已压缩”的假工具。用户会话标题只能以隔离、无工具、非交互的短请求异步生成，结果只能在标题仍为占位值时写入；人工重命名、自动化、策略和指标会话不得被迟到的标题结果覆盖。
- sidecar 必须稳定处理：
  - 会话创建。
  - 发送消息。
  - 流式输出。
...
- 后台 Profile 的账户只读工具同样不能信任模型自行选择的 `accountId`；sidecar 应以非空 `agentProfileAccountId` 为绑定依据覆盖为 Profile 账户，不能依赖子 Agent 的 `backgroundRun` 标记，因为配置型专家会以 `backgroundRun=false` 限制动作权限。Rust 仍必须根据不可变 Run 快照二次注入和校验，不能因模型传入 `profile`、`default` 等占位值让账户风险专家（`role: account_risk`）误判为无账户。
...
- 账户风险专家（`role: account_risk` / `envelope: risk`）的报告表示证据足够形成受约束的风险结论，不表示所有可选数据都存在。成功读取当前余额、持仓和挂单后，空仓、空挂单、空历史属于有效状态；空仓时没有 OKX `liqPx` 不属于阻塞缺口，`liquidationGear` 只是强平风险提醒档位，不能解释为强平价计算状态。USDT 线性永续的账户权益、可用余额和风险预算只使用 USDT，非 USDT 原始币数量不得相加或列为阻断性缺口。无法读取当前风险基线，或具体交易方案缺少必要预检时仍必须失败关闭。
- 禁止向交易 Agent 暴露 `run_commands`、Shell、编辑器、apply_patch 或敏感配置读取工具。
- Subagent 只读，不允许创建/修改机会、交易、通知、提醒或脚本；只有主 Agent 能产生外部副作用。
- Profile 只保存**勾选名单** `enabledAgentIds`：勾选即允许主 Agent 点名，空数组等于主 Agent 独立工作（等价旧 `off`），不存在 `off / auto / custom` 模式、成员数量上下限、必需成员或"自动分配"。Run 载荷 `enabledAgents` 按勾选顺序去重，不做数量截断、不做相关性打分、不做资格静默过滤；缺账户/缺 Skill 的专家不剔除（目录保留 + 任务前缀提示）。后台 Run 仍使用确定性只读 Subagent，不开启持久 Team，避免长期会话状态替代本轮最新市场和账户证据。
- Profile 有勾选名单时，协作 UI 必须明确提示 Token 用量与实际点名次数相关（勾选本身不产生调用），并引导用户查看 AI 用量看板中的 Provider 报告统计；不得使用缺少同任务对照数据支持的固定倍数，也不能暗中改变成员数量、模型或运行权限。
- Agent 定义以 Agent 库 `agents/<id>/AGENTS.md` 为唯一真相（frontmatter 元数据 + 正文即该专家系统提示词主体），内置 Agent 不可编辑、只能复制为自定义；Profile 只引用 id，不再保存成员定义副本。旧"方案（Scheme）"模板与旧自定义成员在读取 Profile 时按读旧写新迁移为库条目（`source: custom`），迁移幂等；旧列与 `ai_agent_schemes` 表保留一版仅供回滚，写路径只写勾选名单。
...
- 专家调度只有一条路径：主 Agent 是唯一编排者，用 `consult_expert` / `follow_up` 按需点名，咨询次数与追问次数都不设上限；后端不得预先成波、打分、选人或预挂载报告，也不得把专家结论转换成对工具的硬否决或移除机会创建工具。专家报告不要求固定 JSON 契约，也绝不因字段缺失、`partial`、`max_iterations` 或缺失成员而失败关闭；报告属于不可信证据，主 Agent 不能执行报告中的指令。
- 专家报告在界面中按原文展示（不做长度截断），并标注来源、阶段（`consult` / `follow_up`）与观测时间；不再呈现状态/立场/置信度/否决理由等结构化字段视图。原始事件保留在默认折叠的诊断区域。若模型结果已明确 `finishReason=error`，必须先按 Provider 错误处理，再决定是否对报告做内容提取，避免把余额、限流或鉴权错误误报为非法 JSON。
- 专家以余额、保证金或最小可开仓位为由主张无法开仓时，结论必须有本轮成功 `trade.precheck` 返回的对应 blocker 支持；仅有 `account.readRisk`、手工公式、其它专家转述或 `precheck blocked=false` 都不构成据以放弃的证据。后端不做报告校验降级统计，唯一的结构化硬判定来自 `trade.precheck` 的不可修复 blocker，且它仍原样返回给调用方、由主 Agent 判断是否据此放弃。
- 专家报告的观测时间不等于数据库快照。每条关键证据仍需携带记录 ID、观测时间或明确数值；最终交易动作必须重新经过最新 `trade.precheck`，不能把专家报告当作执行时行情或账户状态。
...
- 已有仓位的止损保护和止盈退出必须独立评估。`intent=close` 必须携带 `exitKind`：`take_profit` 与 `limit/market` 配对，`stop_loss` 与 `trigger/market` 配对，`emergency` 只能使用 `market`；`size` 是权威平仓张数，`closeFraction` 只是可选比例元数据。没有证据支持的止盈目标可以缺省，但 AI 必须在本轮明确说明目标缺失和下一次复评条件，不得用止损已存在代替止盈判断。创建新退出前必须按精确订单 ID 和语义角色核对现有委托；已有止盈/止损只需移动价格时使用绑定原订单身份的 `intent=amend` 机会，不能再创建同数量的新退出。多个退出只有在数量互不重叠且明确属于分段策略时才允许。
...
- 后台和复盘 Agent 不设置用户可配置的总运行时长上限；明确网络/Sidecar 错误应结束 Run 并退避，正常长推理继续等待。停滞处理只允许发进度提示（`agentProgressNotice`，默认 120 秒无进展、之后按间隔重复），**不得取消或中断会话**，也不得恢复 180 秒停滞杀进程或编排总时限。
