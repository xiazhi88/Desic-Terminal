...
The right-side AI strategy assistant is a multi-turn, runtime-scoped Skill: on every turn it reads only the current unsaved buffer of the bound strategy, audits the complete source against the published strategy contract before proposing or writing a change, may write one complete replacement back to that editor through a revision-checked tool, and must run the bounded deterministic current-source test after each write. The test statically locates every strategy action call, validates its known contract details, then runs the unsaved source once against a natural deterministic K-line fixture with saved parameters and an empty virtual portfolio. It validates only the action actually returned by that run; a statically checked call site may remain runtime-unreached. It is not a strategy-quality judgment, historical backtest, live-market check, or fill simulation. The assistant adds review-oriented comments in the active interface language and cannot save, run a historical backtest, access another strategy, or place an order. A future bundled, platform-sandboxed Python runtime remains a security hardening goal; it must strengthen, not bypass, the Profile confirmation, account permission, risk, audit, idempotency, and order-reconciliation controls.
