import { lazy, Suspense, useCallback, useEffect, useId, useMemo, useRef, useState, type CSSProperties, type Dispatch, type MutableRefObject, type PointerEvent as ReactPointerEvent, type ReactNode, type RefObject, type SetStateAction } from "react";
...
export const IntelligenceWorkspacePage = lazy(() =>
...
export function normalizeAiSkillDefinitions(definitions: AiSkillDefinition[]): AiSkillDefinition[] {
...
export const AI_SKILL_OPTIONS = normalizeAiSkillDefinitions(RAW_AI_SKILL_OPTIONS);
...
function MarketPickerSnapshotQuote({ ticker }: { ticker?: Ticker }) {
  // 每个自选行独立渲染本组件：价格与涨跌% 各自独立跳动；hooks 必须在提前 return 之前调用。
  const last = ticker ? Number(ticker.last) : Number.NaN;
  const open = ticker ? Number(ticker.open24h) : Number.NaN;
  const change = Number.isFinite(last) && Number.isFinite(open) && open > 0 ? ((last / open) - 1) * 100 : null;
  const priceBump = useBump(Number.isFinite(last) ? last : "--");
  const changeBump = useBump(change == null ? "--" : change);
  ...
  return (
    <span className="symbol-quote market-picker__snapshot-quote">
      <strong className={priceBump.className} onAnimationEnd={priceBump.onAnimationEnd}>{Number.isFinite(last) ? fmtPrice(last) : "--"}</strong>
      <em className={clsx(change != null && change > 0 ? "positive" : change != null && change < 0 && "negative", changeBump.className)} onAnimationEnd={changeBump.onAnimationEnd}>
...
function BumpTriggerPrice({ value }: { value: string }) {
  // 订单列表按行 map 渲染，触发价跳动需要每行独立的 hook 状态，故抽出本小组件。
  const bump = useBump(value);
  return <b className={bump.className} onAnimationEnd={bump.onAnimationEnd}>{value}</b>;
}
...
function OrderBookView({ book }: { book: OrderBook | null }) {
  ...
  // .mid-price 自身是 display:flex 布局节点，.bump 的 inline-block 会破坏其内部布局；
  // 且 .mid-price span 命中 12px weak 后代样式（span 包装会污染主价），故用 <b> 包裹价格文本单独跳动。
  const midPriceBump = useBump(visibleBids.length > 0 ? Number(visibleBids[0].px) : "--");
  return (
    <div className="orderbook">
      ...
      <div className="mid-price"><b className={midPriceBump.className} onAnimationEnd={midPriceBump.onAnimationEnd}>{visibleBids.length ? formatOrderBookPrice(visibleBids[0]?.px) : "--"}</b> <span>{t("liveOrderBook")}</span></div>
...
// Plain-text AI message body. Lives here so the i18n source contract
// (test:i18n) keeps finding the data-i18n-skip marker in App.tsx while the AI
// research workspace imports and renders it unchanged.
export function AiMessagePlainText({ message }: { message: AiUiMessage }) {
  return <p data-i18n-skip>{message.text}</p>;
}

export function useRendererMemoryMonitor(scope: string, collect?: () => Record<string, unknown>, intervalMs = 60_000) {
                  ...
                  return (
                    <div className="table-row orders" key={order.ordId || order.clOrdId || order.algoId || order.algoClOrdId}>
                      ...
                      <span className="order-trigger-price">
                        <BumpTriggerPrice value={triggerDisplay} />
                        <em>/</em>
                        <b>{orderDisplay}</b>
