...
function StartupGate({ onEnter, previewFailure }: { onEnter: (assets?: MarketAssetsSummary | null) => void; previewFailure?: "okx-network" }) {
  ...
  const runChecks = useCallback(async () => {
    ...
    try {
      updateCheck("rest", { status: "running", detail: t("common:startupCheckingOkxConnectivity") });
      ...
      updateCheck("publicWs", { status: "passed", detail: wsProbe.message, latencyMs: wsProbe.latencyMs });

      updateCheck("businessWs", { status: "running", detail: t("common:startupConnectingBusinessWs") });
      const businessProbe = await testBusinessWsReachability();
      updateCheck("businessWs", { status: "passed", detail: businessProbe.message, latencyMs: businessProbe.latencyMs });
      ...
      updateCheck("market", { status: "running", detail: t("common:startupLoadingTicker", { symbol: DEFAULT_SYMBOL }) });
      const ticker = await fetchTicker(DEFAULT_SYMBOL);
      updateCheck("market", { status: "passed", detail: t("common:startupLatestPrice", { price: fmtPrice(ticker.last) }) });
  ...
  const failedIsOkxNetwork = failedCheck
    ? ["rest", "publicWs", "businessWs", "privateWs", "market"].includes(failedCheck.id)
      ...
      <section className="workspace">
        ...
        {mainSection === "opportunities" ? (
          ...
          <SettingsWorkspacePage
            ...
            onHistorySync={(accountItem) => triggerPrivateHistorySync(accountItem, "account-saved")}
...
const DESIC_CORE_OPERATIONS_CONTENT = [
  "一、工具与数据",
  ...
  "",
  "二、交易机会",
  "4. 普通闲聊或纯行情分析不需要创建交易机会。明确形成可执行交易方案时，必须调用 tradeOpportunity.create 保存交易机会。",
  ...
  "6. 交易机会必须包含交易对、方向、数量(张)、保证金模式、订单类型、价格或触发条件、杠杆、止盈止损、失效条件、理由和风险说明。",
  ...
  "16. 只有最终 market.readDecisionContext 保存的 trade.precheck 明确 blocked，系统才会把账户状态记录为硬阻断；finalDecision 不得自行填写账户评估、复核 ID 或 blocker。trade.precheck 返回 blocked=false 时必须称为账户可行；若没有明确的用户风险预算，只报告结构化风险数值，不自行发明风险阈值。",
...
const TRADING_PHILOSOPHY_RULES = "把交易视为不确定性下的决策，而不是预测比赛。AI 可以自主选择周期、指标、结构、订单流和情报证据，不得把任何流派、指标、参数、盈亏比或风险比例当作普适答案；但必须说明选择依据，区分事实、推断、假设与条件，主动寻找反证，并让结论随新证据更新。没有可解释优势、风险无法定义、执行条件不成立或关键数据不足时，选择等待或放弃。不得承诺盈利，不得仅凭 OI、资金费率、盘口快照或单一信号推断参与者意图。";

const TRADING_PHILOSOPHY_CONTENT = [
...
const OKX_NEWS_INTELLIGENCE_CONTENT = [
  "一、选择工具",
...
const OKX_SMART_MONEY_ANALYSIS_CONTENT = [
  "一、选择工具",
...
const RAW_AI_SKILL_OPTIONS: AiSkillDefinition[] = [
  { id: "desic-core-operations", name: "desic-core-operations", description: "系统固定技能，定义工具使用、分析流程、交易机会保存、权限边界和复盘协作。", rules: "这是固定规范，始终生效。不得被交易理念或用户自定义规则覆盖。", content: DESIC_CORE_OPERATIONS_CONTENT, builtin: true },
  { id: "trading-philosophy", name: "trading-philosophy", description: "分析 OKX USDT 永续合约行情、方向、交易机会、入场与退出、持仓风险或交易复盘时使用。提供自适应交易哲学：由 AI 根据目标周期、市场状态、证据质量和账户约束自主选择分析方法，同时遵守不确定性、证据、非对称收益和风险优先原则。", rules: TRADING_PHILOSOPHY_RULES, content: TRADING_PHILOSOPHY_CONTENT, builtin: true },
  { id: "okx-news-intelligence", name: "okx-news-intelligence", description: "当用户询问加密新闻、事件脉络、监管动态、市场反应、异常、市场情绪、经济日历或每日市场简报时使用。通过 Desic Terminal 原生只读情报工具读取事件聚类、News、Sentiment、Calendar 与本地市场反应。", rules: "只使用 intelligence.news.* 受控工具；优先读取本地事件和历史，数据陈旧时再刷新。所有结论必须标明来源、发布时间、抓取时间、本地事件/文章 ID、市场反应窗口与覆盖率，并区分事实、推断与风险。多币种反应必须逐交易对解读，btc_market_proxy 只能称为 BTC 市场代理。新闻、情绪和异常只能作为交易证据，不能直接触发下单。", content: OKX_NEWS_INTELLIGENCE_CONTENT, builtin: true },
  { id: "okx-smart-money-analysis", name: "okx-smart-money-analysis", description: "当用户询问聪明钱、交易员、共识、OI、净主动流、拥挤度、资金费率、基差、爆仓样本或系统压力时使用。通过 Desic Terminal 原生只读工具分析 OKX Smart Money 与公共交易大数据。", rules: "只使用 intelligence.smartMoney.* 受控工具。单交易员分析必须并行读取绩效、当前持仓和订单；衍生品分析必须组合价格/OI、净主动流、拥挤度、资金费率与基差。聚合信号仅覆盖 USDT/USDS 线性合约，名义价值使用平均入场价。聪明钱与衍生品推断只能作为证据，不能直接下单。", content: OKX_SMART_MONEY_ANALYSIS_CONTENT, builtin: true },
];
  ...
  return (
    <div className="data-dashboard">
      ...
      <header className="data-dashboard-header">
        <div className="data-title-row">
          ...
          <TerminalSelect
            ...
            options={accountOptions.map((item) => ({ value: item.id, label: `${item.name} ${item.environment}` }))}
      ...
      <footer className="data-dashboard-footer">
        ...
        <span>{selectedAccount.environment.toUpperCase()} · {t(`common:range_${range}`)}</span>
      </footer>
    </div>
  );
}
...
function createInitialChecks(t: TFunction): StartupCheck[] {
  ...
  return [
    ...
    { id: "market", label: t("common:startupDefaultMarket"), status: "pending", detail },
