...
const sessionId = `claude-sidecar-test-${Date.now()}`;
const events = [];
let stderr = "";
let sent = false;
let settled = false;
...
const result = await new Promise((resolveTest, rejectTest) => {
  const timeout = setTimeout(() => rejectTest(new Error("Claude sidecar integration test timed out")), 45_000);
  const finish = (error) => {
    if (settled) return;
    settled = true;
...
console.log("claude cli sidecar MCP and subagent integration passed");
