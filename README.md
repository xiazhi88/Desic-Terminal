...
市场雷达独立扫描 OKX 全部 live USDT 永续市场，不受自选列表限制。它将日线相对强度、实现波动率、成交活跃度、趋势稳定性、流动性与当前市场快照组合成可解释的研究优先级，并保留全市场排名与筛选范围内排名两种口径。

[![Desic Terminal 市场雷达真实应用界面](docs/assets/readme/market-radar.png)](docs/assets/readme/market-radar.png)

- **全市场研究基线**：全部 live USDT 永续准备最多 400 天 UTC 日线，按成交额选出的 Top 100 额外准备 90 天小时线；历史任务在应用启动后后台运行，不依赖打开市场雷达页面。
- **可复现的研究工作流**：支持分类排名、自然语言确定性筛选、多标的比较、市场宽度、1 小时/1 日/7 日排名变化、评分变化归因和 1/5/20 日点时验证。
- **股票永续识别**：使用官方 NASDAQ Trader Symbol Directory 补充证券名称和交易所，并为中文界面提供可缓存的简体/繁体实体名称。
- **AI 原生读取**：内置 `market-radar-research` Skill 与只读 `radar.*` 工具直接读取 SQLite 快照；宽泛的当前市场分析会先检查市场宽度与默认综合排名，不依赖当前 Tab 或可见行。
